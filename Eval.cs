...
internal static class Eval
{
    ...
    internal static ValueT[][] Passed =
        {
            new[]
                {
                    Value.Create(0), Value.Create(1), Value.Create(34), Value.Create(90),
                    Value.Create(214), Value.Create(328)
                },
            new[]
                {
                    Value.Create(7), Value.Create(14), Value.Create(37), Value.Create(63),
                    Value.Create(134), Value.Create(189)
                }
        };
    ...
    internal static BitboardT[] SpaceMask =
        {
            (Bitboard.FileCBB | Bitboard.FileDBB | Bitboard.FileEBB
             | Bitboard.FileFBB)
    ...
    private static void init_eval_info(ColorT Us, Position pos, EvalInfo ei)
    {
        ...
        var b =
            ei.attackedBy[Them, PieceType.KING] = pos.attacks_from_PtS(PieceType.KING, pos.square(PieceType.KING, Them));
        ...
        for (var idx = 0; idx < 16; idx++)
        {
            ...
            var b = Pt == PieceType.BISHOP
                        ? Utils.attacks_bb_PtSBb(
                            PieceType.BISHOP,
                            s,
                            pos.pieces() ^ pos.pieces_CtPt(Us, PieceType.QUEEN))
                        : Pt == PieceType.ROOK
                              ? Utils.attacks_bb_PtSBb(
                                  PieceType.ROOK,
                                  s,
            ...
            if (Pt == PieceType.BISHOP || Pt == PieceType.KNIGHT)
            {
                ...
                {
                    score +=
                        Outpost[Pt == PieceType.BISHOP ? 1 : 0][
                            Bitboard.IsOccupied(ei.attackedBy[Us, PieceType.PAWN], s) ? 1 : 0];
                }

                // Bonus when behind a pawn
                if (Rank.relative_rank_CtSt(Us, s) < Rank.RANK_5
                    && Bitboard.IsOccupied(pos.pieces_Pt(PieceType.PAWN), (s + Square.pawn_push(Us))))
                {
    ...
    private static ScoreT evaluate_threats(ColorT Us, bool DoTrace, Position pos, EvalInfo ei)
    {
        ...
        var defended = (pos.pieces_Ct(Them) ^ pos.pieces_CtPt(Them, PieceType.PAWN))
    ...
    internal static ValueT evaluate(bool DoTrace, Position pos)
    {
        ...
        if (pos.non_pawn_material(Color.WHITE) + pos.non_pawn_material(Color.BLACK) >= 12222)
        {
            score += Score.Multiply(
                evaluate_space(Color.WHITE, pos, ei) - evaluate_space(Color.BLACK, pos, ei),
                Weights[Space]);
        }
            ...
            && (sf == ScaleFactor.SCALE_FACTOR_NORMAL || sf == ScaleFactor.SCALE_FACTOR_ONEPAWN))
        {
            ...
            // pawns are drawish.
                     ...
                     && !pos.pawn_passed(
                         Color.opposite(strongSide),
                         pos.square(PieceType.KING, Color.opposite(strongSide))))
            {
