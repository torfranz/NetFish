...
internal static class Pawns
{
    ...
    private static readonly ScoreT[] Lever =
        {
            ...
            Score.make_score(0, 0), Score.make_score(20, 20),
            Score.make_score(40, 40), Score.make_score(0, 0), Score.make_score(0, 0)
        };
            ...
            new[]
                {
                    new[]
                        {
                            Value.Create(0), Value.Create(67),
                            Value.Create(134), Value.Create(38),
                            Value.Create(32), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(57),
                            Value.Create(139), Value.Create(37),
                            Value.Create(22), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(43),
                            Value.Create(115), Value.Create(43),
                            Value.Create(27), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(68),
                            Value.Create(124), Value.Create(57),
                            Value.Create(32), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        }
                },
            new[]
                {
                    new[]
                        {
                            Value.Create(20), Value.Create(43),
                            Value.Create(100), Value.Create(56),
                            Value.Create(20), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(23), Value.Create(20),
                            Value.Create(98), Value.Create(40),
                            Value.Create(15), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(23), Value.Create(39),
                            Value.Create(103), Value.Create(36),
                            Value.Create(18), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(28), Value.Create(19),
                            Value.Create(108), Value.Create(42),
                            Value.Create(26), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        }
                },
            new[]
                {
                    new[]
                        {
                            Value.Create(0), Value.Create(0),
                            Value.Create(75), Value.Create(14),
                            Value.Create(2), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(0),
                            Value.Create(150), Value.Create(30),
                            Value.Create(4), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(0),
                            Value.Create(160), Value.Create(22),
                            Value.Create(5), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(0),
                            Value.Create(166), Value.Create(24),
                            Value.Create(13), Value.Create(0),
                            Value.Create(0), Value.Create(0)
                        }
                },
            new[]
                {
                    new[]
                        {
                            Value.Create(0), Value.Create(-283),
                            Value.Create(-281), Value.Create(57),
                            Value.Create(31)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(58),
                            Value.Create(141), Value.Create(39),
                            Value.Create(18)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(65),
                            Value.Create(142), Value.Create(48),
                            Value.Create(32)
                        },
                    new[]
                        {
                            Value.Create(0), Value.Create(60),
                            Value.Create(126), Value.Create(51),
                            Value.Create(19)
                        }
                }
        };
    ...
    internal static ScoreT evaluate(ColorT Us, Position pos, Entry e)
    {
        ...
        for (var idx = 0; idx < 16; idx++)
        {
            ...
            var opposed = (theirPawns & Utils.forward_bb(Us, s)) != 0;
            var passed = (theirPawns & Utils.passed_pawn_mask(Us, s)) == 0;
            var lever = (theirPawns & Utils.StepAttacksBB[Piece.make_piece(Us, PieceType.PAWN), s]) != 0;
            var phalanx = neighbours & Utils.rank_bb_St(s);
            var supported = neighbours & Utils.rank_bb_St(s - Up);
            var connected = (supported | phalanx) != 0;
    ...
    internal class Entry
    {
        ...
        {
            return this.semiopenFiles[c] & (leftSide ? (1 << f) - 1 : ~((1 << (f + 1)) - 1));
        }

        internal int pawns_on_same_color_squares(ColorT c, SquareT s)
        {
            return this.pawnsOnSquares[c, Bitboard.IsOccupied(Bitboard.DarkSquares, s) ? 1 : 0];
        }
        ...
        private ScoreT do_king_safety(ColorT Us, Position pos, SquareT ksq)
        {
            ...
            {
                bonus = Value.Create(Math.Max(bonus, shelter_storm(Us, pos, Square.relative_square(Us, Square.SQ_G1))));
            }

            if (pos.can_castle(Movegen.MakeCastling(Us, CastlingSide.QUEEN_SIDE)))
            {
                bonus = Value.Create(Math.Max(bonus, shelter_storm(Us, pos, Square.relative_square(Us, Square.SQ_C1))));
            }
