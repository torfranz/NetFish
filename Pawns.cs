...
public static class Pawns
{
    ...
    {
        Score.make_score(13, 43), Score.make_score(20, 48), Score.make_score(23, 48), Score.make_score(23, 48),
        Score.make_score(23, 48), Score.make_score(23, 48), Score.make_score(20, 48), Score.make_score(13, 43)
    };
    ...
    private static readonly Score[][] Isolated =
    {
        new[]
        {
            Score.make_score(37, 45), Score.make_score(54, 52), Score.make_score(60, 52), Score.make_score(60, 52),
            Score.make_score(60, 52), Score.make_score(60, 52), Score.make_score(54, 52), Score.make_score(37, 45)
        },
        new[]
        {
            Score.make_score(25, 30), Score.make_score(36, 35), Score.make_score(40, 35), Score.make_score(40, 35),
            Score.make_score(40, 35), Score.make_score(40, 35), Score.make_score(36, 35), Score.make_score(25, 30)
        }
    };
...
// Connected pawn bonus by opposed, phalanx, twice supported and rank
    ...
    {
        Score.make_score(0, 0), Score.make_score(0, 0), Score.make_score(0, 0), Score.make_score(0, 0),
        Score.make_score(20, 20), Score.make_score(40, 40), Score.make_score(0, 0), Score.make_score(0, 0)
    };
...
// Center bind bonus: Two pawns controlling the same central square
    ...
    {
        (Bitboard.FileDBB | Bitboard.FileEBB) & (Bitboard.Rank5BB | Bitboard.Rank6BB | Bitboard.Rank7BB),
        (Bitboard.FileDBB | Bitboard.FileEBB) & (Bitboard.Rank4BB | Bitboard.Rank3BB | Bitboard.Rank2BB)
    };
...
// Weakness of our pawn shelter in front of the king by [distance from edge][rank]
    ...
    {
        new[] {new Value(97), new Value(21), new Value(26), new Value(51), new Value(87), new Value(89), new Value(99)},
        new[]
        {new Value(120), new Value(0), new Value(28), new Value(76), new Value(88), new Value(103), new Value(104)},
        new[] {new Value(101), new Value(7), new Value(54), new Value(78), new Value(77), new Value(92), new Value(101)},
        new[] {new Value(80), new Value(11), new Value(44), new Value(68), new Value(87), new Value(90), new Value(119)}
    };
    ...
    private static readonly Value[][][] StormDanger =
    {
        ...
        new[]
        {
            new[] {new Value(0), new Value(-283), new Value(-281), new Value(57), new Value(31)},
            new[] {new Value(0), new Value(58), new Value(141), new Value(39), new Value(18)},
            new[] {new Value(0), new Value(65), new Value(142), new Value(48), new Value(32)},
            new[] {new Value(0), new Value(60), new Value(126), new Value(51), new Value(19)}
        }
    };
    ...
    public static Score evaluate(Color Us, Position pos, Entry e)
    {
        ...
        foreach (var s in pl.TakeWhile(sq => sq != Square.SQ_NONE))
        {
            ...
            if ((passed | isolated | lever | connected)
                || (ourPawns & Utils.pawn_attack_span(Them, s))
            ...
            if (connected)
                score +=
                    Connected[
                        opposed ? 1 : 0, phalanx ? 1 : 0, Bitboard.more_than_one(supported) ? 1 : 0,
    ...
    static int[] Seed = new[] { 0, 6, 15, 10, 57, 75, 135, 258 };
        ...
        for (int opposed = 0; opposed <= 1; ++opposed)
            for (int phalanx = 0; phalanx <= 1; ++phalanx)
                for (int apex = 0; apex <= 1; ++apex)
                    ...
                    {
                        int v = (Seed[r] + (phalanx != 0 ? (Seed[r + 1] - Seed[r]) / 2 : 0)) >> opposed;
                        v += (apex != 0 ? v / 2 : 0);
    ...
    public class Entry
    {
        ...
        private Score do_king_safety(Color Us, Position pos, Square ksq)
        {
            ...
            if (pos.can_castle(Movegen.MakeCastling(Us, CastlingSide.KING_SIDE)))
                bonus = new Value(Math.Max(bonus, shelter_storm(Us, pos, Square.relative_square(Us, Square.SQ_G1))));

            if (pos.can_castle(Movegen.MakeCastling(Us, CastlingSide.QUEEN_SIDE)))
                bonus = new Value(Math.Max(bonus, shelter_storm(Us, pos, Square.relative_square(Us, Square.SQ_C1))));
            ...
            for (var f = center - new File(1); f <= center + new File(1); ++f)
            {
                ...
                safety -= ShelterWeakness[Math.Min(f, File.FILE_H - f)][rkUs]
                          + StormDanger
                              ...
                              [Math.Min(f, File.FILE_H - f)][rkThem];
            }
