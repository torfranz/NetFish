...
public struct Bitboard
{
    ...
    {
        return new Bitboard(Delta.Value == Square.DELTA_N ? this.Value << 8 : Delta.Value == Square.DELTA_S ? this.Value >> 8
              : Delta.Value == Square.DELTA_NE ? (this.Value & ~FileHBB) << 9 : Delta.Value == Square.DELTA_SE ? (this.Value & ~FileHBB) >> 7
              : Delta.Value == Square.DELTA_NW ? (this.Value & ~FileABB) << 7 : Delta.Value == Square.DELTA_SW ? (this.Value & ~FileABB) >> 9
              : 0);
    }
}