...
public static class TranspositionTable
{
    ...
    static uint clusterCount;
    static Cluster[] table;
    //void* mem;
    static byte generation8; // Size must be not bigger than TTEntry::genBound8
    ...
    public static void new_search() { generation8 += 4; } // Lower 2 bits are used by Bound
    public static byte generation() { return generation8; }
    ...
    public static void resize(uint mbSize)
    {
        ...
        uint newClusterCount = (uint)(1 << (int)Utils.msb(new Bitboard(mbSize * 1024 * 1024)));
    ...
    public static TTEntry probe(ulong key, ref bool found)
    {
        ...
        ushort key16 = (ushort)(key >> 48);  // Use the high 16 bits as key inside the cluster
        ...
        TTEntry replace = cluster.entry[0];
          for (int i = 1; i<ClusterSize; ++i)
              ...
              if (  replace.depth8 - ((259 + generation8 - replace.genBound8) & 0xFC) * 2 * Depth.ONE_PLY
                  > cluster.entry[i].depth8 - ((259 + generation8 - cluster.entry[i].genBound8) & 0xFC) * 2 * Depth.ONE_PLY)
